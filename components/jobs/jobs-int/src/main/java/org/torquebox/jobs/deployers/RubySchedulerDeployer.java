...
public class RubySchedulerDeployer extends AbstractDeployer {
    ...
    private SchedulerProxy buildScheduler(DeploymentUnit unit, boolean singleton) {
        ...
        SchedulerProxy proxy = new SchedulerProxy();
        builder.addPropertyMetaData( "schedulerProxy", proxy );
    ...
    public boolean isClustered() {
    	...
    	if (null != controller) {
    		isClustered = controller.getContext("HASingletonDeployer", ControllerState.INSTANTIATED, false) != null; 
    	} else {
    ...
    public void deploy(DeploymentUnit unit) throws DeploymentException {
        ...
        if (this.isClustered()) {
            log.info( "Deploying clustered scheduler: " + unit );
            SchedulerProxy singletonProxy = null;
            SchedulerProxy regularProxy = null;
            if ( jobTypes.singletonJobs ) { 
                singletonProxy = this.buildScheduler( unit, true  ); 
            }
            if ( jobTypes.regularJobs ) { 
                regularProxy = this.buildScheduler( unit, false ); 
            }
            ...
            for (ScheduledJobMetaData each : allMetaData) { 
                ...
                each.setSchedulerProxy( each.isSingleton() ? singletonProxy : regularProxy );
            }
        } else {
            log.info( "Deploying scheduler: " + unit );
            SchedulerProxy proxy = this.buildScheduler( unit, false );
            for (ScheduledJobMetaData each : allMetaData) { 
                each.setSchedulerProxy( proxy );
            }
